...
def verify_excluded_layers(model):
    """Check that downsample convs and final fc layer are not pruned."""
    for name, module in model.named_modules():
        if ("downsample" in name and isinstance(module, nn.Conv2d)) or isinstance(module, nn.Linear):
            if hasattr(module, "weight_mask"):
...
def main(args):
    ...
    lottery_model = models.resnet18(num_classes=10).to(device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Iterative Pruning with Lottery Ticket Hypothesis (ResNet18 CIFAR10)")
    parser.add_argument(
        "--percent",
        type=float,
        default=0.2,
